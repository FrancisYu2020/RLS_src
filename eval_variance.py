...
def test_model(model, mode, cross_val_loader, internal_val_loader):
    ...
    if cross_val_data is not None:
        ...
        log_data.update({"cross_loss": val_loss, "cross_f1": f1, "cross_f0.5":fprec, "cross_precision":precision, "cross_recall":recall, "cross_miou": miou})
    if internal_val_data is not None:
        val_loss, y_pred, y_true = val_loop(args, model, internal_val_loader, criterion)
        precision, recall, f1, fprec, miou = compute_metrics(y_pred, y_true)
        log_data.update({"internal_loss": val_loss, "internal_f1": f1, "internal_f0.5":fprec, "internal_precision":precision, "internal_recall":recall, "internal_miou": miou})
...
def test_all_checkpoints(model, mode):
    assert mode in ['cross', 'internal']
    results_path = os.path.join(args.checkpoint_dir, f'test_{mode}.json')
    if os.path.exists(results_path):
        print(f'test_{mode}.json already exist:')
        print(json.load(results_path))
        print()
        return
    test_model(model, mode)
...
for i in range(args.num_val_sets):
    if cross_val_data is not None:
        cross_val_datasets.append(RLSDataset(args, cross_val_data[], cross_val_label, transform=val_transform))
        cross_val_loaders.append(DataLoader(dataset=cross_val_dataset, num_workers=args.num_workers, batch_size=args.batch_size, shuffle=False, pin_memory=True))
    if internal_val_data is not None:
        internal_val_dataset = RLSDataset(args, internal_val_data, internal_val_label, transform=val_transform)
        internal_val_loader = DataLoader(dataset=internal_val_dataset, num_workers=args.num_workers, batch_size=args.batch_size, shuffle=False, pin_memory=True)
